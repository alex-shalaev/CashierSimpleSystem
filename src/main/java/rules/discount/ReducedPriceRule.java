...
@Setter
@AllArgsConstructor
public class ReducedPriceRule implements DiscountRule {
    ...
    public double calculateDiscount(Map<String, Integer> productQuantity,
        ...
        for (Map.Entry<String, Integer> productItem : productQuantity.entrySet()) {
            ...
            if (code.equals(ProductEnum.STRAWBERRIES_CODE.getValue())) {
                int quantity = productItem.getValue();
                if (quantity >= numToBuy) {
                    double originalPrice = products.get(code).getPrice() * quantity;
                    double newPrice = getNewPrice() * quantity;
